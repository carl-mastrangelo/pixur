...
func TestUpdateUserTaskDifferentUser(t *testing.T) {
	...
	task := &UpdateUserTask{
		...
		NewCapability: append(ou.User.Capability, schema.User_USER_CREATE),
	}
...
func TestUpdateUserTaskSameUserDefault(t *testing.T) {
	...
	task := &UpdateUserTask{
		...
		NewCapability: append(u.User.Capability, schema.User_USER_CREATE),
	}
...
func TestUpdateUserTaskMissingCap(t *testing.T) {
	...
	task := &UpdateUserTask{
		...
		Ctx:           CtxFromUserID(context.Background(), su.User.UserId),
		NewCapability: make([]schema.User_Capability, 0),
	}
